#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# Wind Turbine Digital Twin - Prediction Script using Saved MLP Model (Direct Input)                          # # <-- Updated Title
# By [Wind Turbine Digital Twins]                                                                               #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# Description:                                                                                                  #
# This script loads a previously trained MLP pipeline (saved as a .pkl file)                                    # # <-- Updated Description
# and uses it to predict RPM, Voltage, Power, and Current based on input values                                 #
# defined directly within the script. Only the predicted values are printed to the console.                     #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
# --- File Paths ---
# *** IMPORTANT: Specify the correct path to your saved MLP model file ***
# Based on the training script naming convention, assuming weather and linaccel were included:
saved_model_filename = 'rpm_volt_pow_curr_multi_mlp_model_fanflags_weather_linaccel.pkl' # <--- VERIFY THIS FILENAME
...
# output_predictions_filename = 'mlp_single_prediction_output.csv'

# --- Feature & Target Definitions (MUST MATCH the training script for the loaded model) ---
...
numerical_features = [
    ...
    'orientation_heading',      # Note: Altitude was added later in MLP script, verify which version was saved
    'orientation_roll',
    'orientation_pitch',
    'weatherstation_altitude',             # Included based on training script provided
    'low_fan',
    'med_fan',
    'high_fan',
    'linear_acceleration_lx',   # Included based on training script provided
    'linear_acceleration_ly',   # Included based on training script provided
    'linear_acceleration_lz'    # Included based on training script provided
]
...
# 3. Load Model                                                                                                   #
#-----------------------------------------------------------------------------------------------------------------#
print(f"Loading saved MLP model pipeline from: {saved_model_filename}") # Updated print
...
for col in numerical_features:
    try:
        ...
        exit()
         ...
         print(f"\n--- ERROR ---")
         print(f"Feature '{col}' defined in numerical_features list but not found in input_data_dict.")
         exit()
...
# 5. Make Prediction                                                                                              #
#-----------------------------------------------------------------------------------------------------------------#
print("\nMaking prediction using the loaded MLP model...") # Updated print
start_predict_time = time.time()

try:
    # Use the loaded pipeline to make predictions on the single input row
    # Need to ensure the columns passed match exactly what the pipeline expects
    # The preprocessor inside the pipeline knows which columns to transform
    predictions_array = model_pipeline.predict(input_df[numerical_features]) # Pass the DataFrame slice
except Exception as e:
    print(f"\n--- ERROR ---")
    print(f"An error occurred during prediction: {e}")
    # Print columns expected by the scaler vs columns provided
    try:
        expected_cols = model_pipeline.named_steps['preprocessor'].transformers_[0][2]
        print(f"Preprocessor expected columns: {expected_cols}")
        print(f"Columns provided: {input_df[numerical_features].columns.tolist()}")
    except Exception as e2:
        print(f"Could not retrieve expected columns from preprocessor: {e2}")
    exit()
...
# --- Display Results in Console (Predictions Only) ---
print("\n--- Prediction Results ---")
